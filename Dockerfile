...
RUN dpkg --add-architecture i386 && \
	apt-get update && \
	apt-get -qy upgrade && apt-get -qy install wget software-properties-common apt-transport-https openssh-server xauth cabextract winbind squashfs-tools pulseaudio sudo x11-apps xfce4 cups joe xfce4-terminal xrdp xorgxrdp xvfb socat x11vnc firefox && \
	...
	apt-get -qy install --install-recommends winehq-staging && \
