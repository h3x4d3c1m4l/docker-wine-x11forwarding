...
RUN dpkg --add-architecture i386 && \
	apt-get update && \
	apt-get -qy upgrade && apt-get -qy install wget software-properties-common apt-transport-https openssh-server xauth cabextract winbind squashfs-tools xvfb x11vnc xserver-xephyr websockify dbus-x11 pulseaudio sudo xserver-xorg-video-dummy x11-apps xfce4 cups joe && \
